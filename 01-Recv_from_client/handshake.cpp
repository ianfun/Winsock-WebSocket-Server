...
BOOL HashHanshake(void* key, ULONG length, char *buf/*31 bytes*/){
    BYTE   sha1_o[20]{};
    static_assert(sizeof(sha1_o)==20);
    if ((BCryptHashData(hHash, (PUCHAR)key, length, 0)) < 0)
        return FALSE;
    if ((BCryptFinishHash(hHash, sha1_o, sizeof(sha1_o), 0)) < 0)
        return FALSE;
    DWORD size=31;
    return CryptBinaryToStringA(sha1_o, sizeof(sha1_o), CRYPT_STRING_BASE64, buf, &size);
}
